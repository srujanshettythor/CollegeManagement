                ...
                <div class="card">
                  <div class="card-body ">
                    <div class="d-flex" style="gap:30px">
                      ...
                      <div class="w-100">
                      <form action="view_student_mark.php?s_id=<?php echo $sid ?>" method="POST">
                        <select name="exam" required class="form-control">
                          <option value="" hidden>Choose Exam</option>
            ...
            ?>
                    <div class="table-responsive">
                      <table class="table">
                        ...
                        <?php
                        $i=1;
                            $stmt=$admin->ret("SELECT * FROM `marks` inner join `student` on student.s_id=marks.s_id inner join `staff` on staff.st_id=marks.tr_id inner join `subjects` on subjects.sb_id=marks.sb_id where `m_exam`='$exam' AND marks.s_id='$sid' and marks.tr_id='$stid'");
                           while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {  
                        ?>
                          <tr>
                              <td><?php echo $i++ ?></td>
                              <td><?php echo $row['m_posted_date']; ?></td>
                              <td><?php echo $row['st_name']; ?></td>
                              <td><?php echo $row['sb_name']; ?></td>
                              <td><?php echo $row['m_total_marks']; ?></td>
                              <td><?php echo $row['m_marks_obtained']; ?></td>
                              <td><?php echo $row['m_grade']; ?></td>
                              <td><?php echo $row['m_percentage']; ?></td>
                              <td><?php echo $row['m_status']; ?></td>
                              <td>
                                <div class="mt-1">
                                <a data-toggle="modal" data-target="#exampleModa<?php echo $row['m_id'] ?>" data-whatever="@mdo">
                                  <i class="mdi mdi-border-color text-secondary" style="font-size:25px"></i>
                                </a>
                              </div>
                              </td>
                          </tr>
                          <div class="modal fade" id="exampleModa<?php echo $row['m_id'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                          <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <!-- <form action="controller/staff_attendence.php" method="POST"> -->
                                <div class="modal-header">
                                  <h5 class="modal-title" id="exampleModalLabel">Update Marks
                                  </h5>
                                ...
                                <div class="modal-body">
                                  <div class="container">
                                    <form method="post" action="controller/bupdate_marks.php">
                                      <input type="hidden" name="mid" value="<?php echo $row['m_id'] ?>" id="mid">
                                      <input type="hidden" name="sid" value="<?php echo $row['s_id'] ?>" id="mid">
                                      <label class="d-flex" style="gap:10px">
                                        <span class="mt-2">Marks obtained </span>
                                        <input type="number" min="0" name="marx" required class="form-control w-50" />
